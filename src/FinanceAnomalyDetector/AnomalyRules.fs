...
module AnomalyRules =
    ...
    let checkCategoryAverage (expense: Expense) (history: Expense list) =
        ...
        if categoryHistory.Length < 3 then None
            ...
            if avg = 0m then None
                ...
                Some { Id = 0; ExpenseId = expense.Id; Score = score; Severity = getSeverity score; Reason = sprintf "Expense is %.1fx higher than the category average." (expense.Amount / avg); Recommendation = "Review this transaction and check if it was intentional."; DetectedAt = DateTime.UtcNow }
            else None
    ...
    let checkMerchantSpike (expense: Expense) (history: Expense list) =
        ...
        if merchantHistory.Length < 2 then None
            ...
            if avg = 0m then None
                ...
                Some { Id = 0; ExpenseId = expense.Id; Score = score; Severity = getSeverity score; Reason = sprintf "Unusually large amount for merchant %s." expense.Merchant; Recommendation = "Verify the purchase amount."; DetectedAt = DateTime.UtcNow }
            else None
    ...
    let checkCategoryChange (expense: Expense) (history: Expense list) =
        ...
        if merchantHistory.Length < 3 then None
            ...
            if mainCategory <> expense.Category then
                let score = 65
                Some { Id = 0; ExpenseId = expense.Id; Score = score; Severity = getSeverity score; Reason = sprintf "Merchant '%s' is usually in '%s' but this is '%s'." expense.Merchant mainCategory expense.Category; Recommendation = "Check for misclassification."; DetectedAt = DateTime.UtcNow }
            else None
    ...
    let checkNightSpending (expense: Expense) =
        ...
        if hour >= 1 && hour <= 4 then
            let score = 50
            Some { Id = 0; ExpenseId = expense.Id; Score = score; Severity = getSeverity score; Reason = "Transaction occurred late at night (1 AM - 4 AM)."; Recommendation = "Confirm if you made this purchase."; DetectedAt = DateTime.UtcNow }
        else None
